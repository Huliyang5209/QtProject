...
void http_conn1::initmysql_result(sql_connection_pool* sql_pool)
{
    MYSQL *mysql=NULL;
    sql_connection_pool_RAII mysqlcon(&mysql,sql_pool);
...
int addfd(int epollfd,int m_fd,bool is_EPOLLONESHOT,int m_TRIGMode)
{
    epoll_event event;
    event.data.fd=m_fd;
    ...
    epoll_ctl(epollfd,m_fd,EPOLL_CTL_ADD,&event);
    SetNonBlocking(m_fd);
...
void http_conn1::init(int sockfd,const sockaddr_in &addr,char* root,int TRIGMode ,int close_log,string user,string passwd,string dbname)
{
    m_sockfd = sockfd;
    m_address = addr;
...
void removefd(int epollfd,int m_fd)
{
    epoll_ctl(epollfd,EPOLL_CTL_DEL,m_fd,0);
    close(m_fd);
}
...
//读取客户端数据，要求有ET模式,并有意外情况处理---提前关闭？
bool http_conn1::read_one()
{
    if(m_read_idx>=READ_BUFFER_SIZE)//缓冲区已满
    {
        return false;
    }
    int m_hadread_buf_size;
    if(m_TRIGMode==0)//LT
    {
        m_hadread_buf_size=recv(m_sockfd, m_read_buf + m_read_idx, READ_BUFFER_SIZE - m_read_idx, 0);
        m_read_idx+=m_hadread_buf_size;

        if (m_hadread_buf_size<= 0)//
        {
            return false;
        }
    }
    else//ET
    {
        while (true)
        {
            m_hadread_buf_size=recv(m_sockfd, m_read_buf + m_read_idx, READ_BUFFER_SIZE - m_read_idx, 0);
            m_read_idx+=m_hadread_buf_size;
            if(m_hadread_buf_size==-1)//提前关闭连接
            {
                 if (errno == EAGAIN || errno == EWOULDBLOCK)
                    break;
                return false;
            }
            else if(m_hadread_buf_size==0)
            {
                return false;
            }
...
http_conn1::HTTP_CODE http_conn1::parse_contents(char *text)
{
...
void http_conn1::close_conn(bool real_close )
{
